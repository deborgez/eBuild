'use client'
// src/components/dashboard/anexar-comprovante-pagamento-modal.tsx
import { useState } from 'react'

export function AnexarComprovantePagamentoModal({ lancamentoId, jaAnexado, onSalvo }: {
  lancamentoId: string; jaAnexado?: boolean; onSalvo: () => void
}) {
  ...
  async function handleSalvar() {
    ...
    if (comprovantePagamentoUrl) {
      await fetch(`/api/lancamentos/${lancamentoId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ comprovantePagamentoUrl }),
      })
    }
  ...
  return (
    <>
      <button onClick={() => setAberto(true)} className="btn-secondary text-xs py-1 px-2"
        title={jaAnexado ? 'Substituir comprovante de pagamento' : 'Anexar comprovante de pagamento'}>
        {jaAnexado ? '🧾' : '📎 Comprovante'}
      </button>
