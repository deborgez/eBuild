...
export function Sidebar({ user }: { user: any }) {
  ...
  return (
    <aside className="w-60 flex-shrink-0 flex flex-col border-r transition-colors"
          ...
          <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
            ...
            <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                d="M2.25 21h19.5m-18-18v18m10.5-18v18" />
            </svg>
          </div>
          <div>
            <p className="font-bold text-sm leading-tight" style={{ color: 'var(--color-text-primary)' }}>eBuild</p>
            <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>Gestão de Obras</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-3 border-t space-y-1" style={{ borderColor: 'var(--color-border)' }}>
        {/* Dark mode toggle */}
        <button onClick={toggleTheme} className="sidebar-link w-full justify-between">
          <div className="flex items-center gap-3">
            {isDark ? (
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
          ...
          <div className="w-9 h-5 rounded-full transition-colors relative flex-shrink-0"
            style={{ backgroundColor: isDark ? 'var(--color-brand)' : 'var(--color-border)' }}>
        ...
        {/* Perfil */}
        <Link href="/configuracoes/perfil" className={cn('sidebar-link w-full', pathname === '/configuracoes/perfil' && 'active')}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs truncate" style={{ color: 'var(--color-text-muted)' }}>Ver perfil</p>
          </div>
        </Link>

        {/* Sair */}
        <button onClick={() => signOut({ callbackUrl: '/login' })}
          className="sidebar-link w-full" style={{ color: '#dc2626' }}>
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
